...
function main() {
  ...
  if (fileNames && fileNames.length > 0) {
    mergedContent += fileNames.sort(customSort).map(line => `- [${line.replace('.md', '')}](/线路/${line.replace('.md', '')}/${line})`).join('\n')
  }
  ...
  readDirectory('./站点');
  if (fileNames && fileNames.length > 0) {
    mergedContent += fileNames.sort(customSort).map(station => `- [${station.replace('.md', '')}](/站点/${station.replace('.md', '')}/${station})`).join('\n')
  }
